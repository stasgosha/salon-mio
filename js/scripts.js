document.addEventListener('DOMContentLoaded', function(){
	...
	$('.big-slider-wrapper').each(function(i, el){
		$(el).on('init reInit beforeChange', function(e, s, c, n){
			let slide = (n ? n : 0) + 1;

			$(el).find('.big-slider-nav .pages').html(`<strong>${slide}</strong> / ${s.slideCount}`);
		})

		$(el).find('.big-slider').slick({
			slidesToShow: 1,
			slidesToScroll: 1,
			speed: 800,
			arrows: true,
			...arrowsButtons,
			appendArrows: $(el).find('.big-slider-nav .arrows'),
			dots: false,
			// infinite: true,
			// vertical: true
		});
	...
	$('.team-slider-component').each(function(i, el){
		// console.log($(el).find('.arrows'));
		$(el).find('.team-slider').slick({
			slidesToShow: 4,
			slidesToScroll: 1,
			speed: 800,
			arrows: true,
			appendArrows: $(el).find('.arrows'),
			dots: true,
		...
		equalSlideHeight($(el).find('.team-slider'));
	});
	...
	equalSlideHeight('.proposals-slider');
	...
	$('.certs-slider').slick({
		...
		dots: false,
		infinite: true,
		appendArrows: $(window).width() > 767 ? $('.cert-slider-arrows') : $('.certs-slider'),
		responsive: [
			{
				breakpoint: 1200,
				settings: {
	...
	equalSlideHeight('.certs-slider');


	// Brands
	$('.brands-list .hidden').slideUp();
	$('.js-show-all-brands').click(function(e){
		e.preventDefault();

		$('.brands-list .hidden').slideDown(300);
	...
	$("[data-tab]").click(function(e){
		...
		if (dest == "show-all") {
			var newDest = $(this).closest('.tabs-nav').find('[data-tab^="#"]').eq(0).data('tab');

			$(newDest).closest('.tabs-container').find('.tab').stop().fadeIn(300);
		} else{
